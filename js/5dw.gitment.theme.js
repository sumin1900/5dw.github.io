const dw_gitment_theme = {
  function renderHeader({ meta, user, reactions }, instance) {
    ...
    likeButton.innerHTML = `
      ...
      ${ meta.reactions && meta.reactions.heart
        ? ` • <strong>${meta.reactions.heart}</strong> 次被喜欢`
        : ''
      }
    `
  ...
  function renderComments({ meta, comments, commentReactions, currentPage, user, error }, instance) {
    ...
    if (error) {
      ...
      if (error === NOT_INITIALIZED_ERROR
        ...
        initButton.innerText = '请管理员初始化评论先'
      ...
      emptyBlock.innerText = '还没有评论，快来抢沙发！'
        ...
        <div class="gitment-comment-main">
          <div class="gitment-comment-header">
            ...
            ${ createDate.toString() !== updateDate.toString()
              ? ` • <span title="修改于 ${updateDate}">修改过</span>`
              : ''
            }
  ...
  function renderEditor({ user, error }, instance) {
    ...
    const disabledTip = user.login ? '' : '登录Github之后才可评论'
    container.innerHTML = `
        ${ user.login
        ...
        <div class="gitment-editor-header">
          ...
          <div class="gitment-editor-login">
            ${ user.login
              ...
              : user.isLoggingIn
                ? '登录中...'
                : `<a class="gitment-editor-login-link" href="${instance.loginLink}">登录</a>到Github`
            }
          </div>
        </div>
        <div class="gitment-editor-body">
          <div class="gitment-editor-write-field">
            <textarea placeholder="感谢您在这里留下评论" title="${disabledTip}" ${shouldDisable}></textarea>
          </div>
    ...
    previewTab.onclick = () => {
      ...
      if (!content) {
        preview.innerText = '还什么都没有...'
        return
      }

      preview.innerText = '努力产生预览中...'
